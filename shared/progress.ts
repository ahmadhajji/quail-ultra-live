...
export function deriveBlockMode(_block: Partial<BlockRecord> & { timelimit?: number; showans?: boolean }): Mode {
  return 'tutor'
}
...
export function replayBuckets(progress: ProgressRecord, qbankinfo: Pick<QbankInfo, 'index' | 'tagnames'>): void {
...
export function startBlock(
  qbankinfo: Pick<QbankInfo, 'progress' | 'tagnames' | 'index' | 'choices'>,
  blockqlist: string[],
  preferences: Partial<StartBlockPreferences> = {}
): string {
  ...
  const mode: Mode = preferences.mode ?? 'tutor'
...
export function deleteBlock(qbankinfo: Pick<QbankInfo, 'progress' | 'tagnames' | 'index'>, blockKey: string): void {
  ...
  for (const qid of block.blockqlist) {
    if (isInBucket(qbankinfo.progress, qbankinfo, qid, 'incorrects')) {
      removeFromBucket(qbankinfo.progress, qbankinfo, qid, 'incorrects')
    }
    if (isInBucket(qbankinfo.progress, qbankinfo, qid, 'flagged')) {
      removeFromBucket(qbankinfo.progress, qbankinfo, qid, 'flagged')
    }
    addToBucket(qbankinfo.progress, qbankinfo, qid, 'unused')
  }
