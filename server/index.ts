...
const { legacyPageRedirectTarget, routePathFor } = require('./routes')
...
function redirectToSpaPath(req, res, targetPath) {
...
app.get('/:page(overview|newblock|previousblocks|examview|admin)', function spaPages(_req, res) {
  res.sendFile(path.join(DIST_DIR, 'index.html'))
})

app.get('/:page(overview|newblock|previousblocks|examview|admin).html', function htmlPages(req, res) {
  redirectToSpaPath(req, res, legacyPageRedirectTarget(req.params.page))
})

app.get(['/loadbank', '/loadbank.html'], function loadbankRedirect(req, res) {
