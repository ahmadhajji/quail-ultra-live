...
class TerminalReviewUI:
    ...
    def _render_question(self, question: ExtractedQuestion) -> Panel:
        ...
        if question.fact_check:
            ...
            proposal = str(
                question.proposed_correct_answer or question.fact_check.get("recommended_answer", "")
            ).strip()
            if proposal:
        ...
        if question.comments:
            ...
            for comment in question.comments[:3]:
                content_parts.append(f"  • {comment.get('author', 'Unknown')}: {comment.get('content', '')[:120]}")
            content_parts.append("")

        content_parts.append(
            f"[dim]Images: {len(question.images)} question / {len(question.explanation_images)} explanation[/dim]"
        )
    ...
    def _edit_question(self, question: ExtractedQuestion) -> dict:
        ...
        self.console.print(
            question.question_stem[:300] + "..." if len(question.question_stem) > 300 else question.question_stem
        )
    ...
    def run_review(
        self, questions: list[ExtractedQuestion], save_callback: Optional[Callable] = None
    ) -> list[ReviewResult]:
        """
