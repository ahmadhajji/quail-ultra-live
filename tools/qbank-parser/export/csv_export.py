...
def export_to_csv(
    questions: list[ExtractedQuestion], output_path: str | Path, review_results: Optional[list[ReviewResult]] = None
) -> Path:
    """
...
def export_to_json(
    ...
    output_data = {
        ...
        "total_slides": len(
            {int(getattr(q, "slide_number", 0)) for q in questions if int(getattr(q, "slide_number", 0)) > 0}
        ),
...
def get_confirmed_questions(
    questions: list[ExtractedQuestion], review_results: list[ReviewResult]
